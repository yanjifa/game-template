...
(function () {
    ...
    var devices = [
        { name: "Xiao Mi 10 Pro", width: 540, height: 1170, ratio: 1, safeWidth: 540, safeHeight: 1125 },
    ...
    function setCookie(name, value, days) {
        // cookie will be saved for 30 days
        days = days || 30;
    ...
    function getEmulatedScreenSize() {
        ...
        return {
            width: rotated ? h : w,
            height: rotated ? w : h,
            safeWidth: rotated ? sh : sw,
            safeHeight: rotated ? sw : sh,
            ratio: sRatio,
        };
    }
    ...
    function onload() {
        ...
        cc.sys.getSafeAreaRect = function () {
            ...
            const screenSize = getEmulatedScreenSize();
            if (!isFullScreen() && screenSize.safeWidth && screenSize.safeHeight) {
                const resolution = cc.Canvas.instance.designResolution;
                if (rotated) {
                    const offset = (resolution.width / screenSize.width) * (screenSize.width - screenSize.safeWidth);
                    return cc.rect(offset, 0, winSize.width - offset, winSize.height);
                } else {
                    const offset = (resolution.height / screenSize.height) * (screenSize.height - screenSize.safeHeight);
                    return cc.rect(0, -offset, winSize.width, winSize.height - offset);
                }
            } else {
